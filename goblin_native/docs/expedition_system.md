...
```
通常戦闘: 出現した敵ごとに round(敵Lv * 1.8 * 種族係数) を合計
ボス戦: isBoss=true の敵は round(敵Lv * 9.6 * 種族係数)、随伴敵は通常戦闘と同じ
```

敵JSONの `exp` は互換用で、実際の遠征経験値は `calculateEnemyExp` で算出する。
...
#### レベルアップ必要経験値（ExperienceSystem）

参考ゲームの「次のLvアップまでの経験値」表示から推定した近似式を使用する。

```
必要経験値 = round(0.00032188652886324764 * (level + 9.17) ^ 4.735402233154439)
```
...
|---:|---:|---:|---|
| 1 | 2 | 19 | 確定 |
| 11 | 12 | 479↑ | Lvアップ後の取得済み経験値が不明なため下限推定 |
| 23 | 24 | 4,514↑ | Lvアップ後の取得済み経験値が不明なため下限推定 |
| 36 | 37 | 22,000 | 確定 |
| 41 | 42 | 34,300↑ | Lvアップ後の取得済み経験値が不明なため下限推定 |
| 44 | 45 | 47,130↑ | Lvアップ後の取得済み経験値が不明なため下限推定 |
| 45 | 46 | 51,000 | 確定 |
| 46 | 47 | 57,880 | ほぼ確定。キリよく58,000の可能性あり |
...
| 3 | orc_camp_1/2/3, undead_ruins_1/2/3 |
| 4 | human_village |
| 5 | wolf_grassland_1, dwarf_mine_1/2/3, elf_forest_1/2/3 |
| 6 | lizardman_swamp_1/2/3, subjugation_force_1/2/3, spider_forest_1, dead_grave_1/2/3, harpy_cliff_1, troll_canyon_1/2/3 |
