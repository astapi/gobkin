...
```
1. 継承元の設定
   「群れを増やす」でプレイヤーが1体を固定し、もう1体は拠点からランダム選出
   拠点にマルクしかいない場合は単独
   ↓
2. 因子収集
   継承元とランダム個体がサイクル開始時に持つ因子IDを収集（重複排除）
   ↓
3. 引き継ぎ判定
   各因子について inheritProbability の確率で個別判定
   ↓
4. 亜種化判定（引き継いだ因子がある場合のみ）
   子の＋値が variantConfig.minPlusValue 以上の因子だけを順番にチェックし、variantConfig.probability で判定
   最初に成功した1種のみ適用
```
...
【遠征】ボス戦勝利
  ...
  │
  ▼
【ゴブリン誕生】「群れを増やす」の時間到達時
  │
  ├─ 固定した継承元とランダム選出個体の因子を収集
  ├─ 各因子の引き継ぎ判定
  │   └─ inheritProbability で個別判定
  ├─ 引き継いだ因子で亜種化判定
  │   └─ 最低＋値を満たす因子を variantConfig.probability で判定（最初の成功のみ）
  │
  ├─ 亜種化成功 → 亜種ゴブリン誕生
  │   ├─ 種族名・アバター変更
  │   ├─ 因子効果 + 追加効果
