  ...
  return (
    <SafeAreaView style={styles.container} edges={['top', 'left', 'right', 'bottom']}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.contentContainer}>
        ...
        <Text style={styles.screenLead}>運営状況の確認と施設機能の利用をここで行います。</Text>

        <View style={styles.heroCard}>
          <StatusRow label="ランク" value={`ランク${rank}`} />
          <StatusRow label="拠点" value={baseLocationName} />
          <StatusRow label="収容数" value={`${goblins.length} / ${maxGoblins}`} />
          <StatusRow label="最大PT数" value={`${maxParties}`} />
          <StatusRow label="個体値補正" value={`+${ivBonus}`} />
        </View>

        <View style={styles.card}>
          <Text style={styles.cardTitle}>拠点メニュー</Text>

          <TouchableOpacity style={styles.menuButton} onPress={() => router.push('/base/upgrade')}>
            <View style={styles.menuButtonTextGroup}>
              <Text style={styles.menuButtonTitle}>拠点拡張</Text>
              <Text style={styles.menuButtonDescription}>ランクアップ条件、必要資金、拡張効果を確認します。</Text>
            </View>
          ...
          <TouchableOpacity style={styles.menuButton} onPress={() => router.push('/base/training')}>
            <View style={styles.menuButtonTextGroup}>
              <Text style={styles.menuButtonTitle}>訓練所</Text>
              <Text style={styles.menuButtonDescription}>ゴブリンが訓練しジョブを身につける</Text>
            </View>
            <Text style={styles.menuButtonArrow}>›</Text>
          </TouchableOpacity>
        </View>
