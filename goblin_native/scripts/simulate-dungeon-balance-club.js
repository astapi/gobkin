...
function createClubEquipment(goblin) {
  const slots = EquipmentService.getAvailableSlots(goblin)
  return Array.from({ length: slots }, (_, slotIndex) => ({
    id: `sim_club_${goblin.id}_${slotIndex}`,
    templateId: CLUB_TEMPLATE_ID,
    slotIndex,
    goblinId: goblin.id,
  }))
}
...
function applyClubEquipment(goblin) {
  const equippedItems = createClubEquipment(goblin)
...
function applyClubEquipmentToParty(goblins, maxPartySize) {
  return goblins.slice(0, maxPartySize).map(applyClubEquipment)
}

async function runExpedition(areaId, goblins, seed, maxPartySize) {
  const engine = new ExpeditionEngine(seed)
  const party = applyClubEquipmentToParty(goblins, maxPartySize)
...
function printSingleResult(result) {
  ...
  console.log(`装備条件: PT全員が解放済み装備枠すべてにこんぼう(${CLUB_TEMPLATE_ID})を装備`)
