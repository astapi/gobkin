...
const TARGET_TITLE = '放置ダンジョンRPG | ゴブリンレガシー';
...
function validateConfig(config) {
  ...
  if ([...TARGET_TITLE].length > 30) {
    throw new Error(`App Store title exceeds 30 characters: ${TARGET_TITLE}`);
  }
}
...
async function main() {
  ...
  console.log('App Store Connect アプリ名更新');
  console.log(`対象ロケール: ${TARGET_LOCALE}`);
  console.log(`更新後タイトル: ${TARGET_TITLE}`);
  console.log(`文字数: ${[...TARGET_TITLE].length}/30\n`);
  ...
  const currentTitle = target.attributes.name ?? '';
  if (currentTitle === TARGET_TITLE) {
    console.log('既に目的のタイトルです。更新は不要です。');
    return;
  }
  ...
  console.log('更新完了');
  console.log(`変更前: ${currentTitle || '(なし)'}`);
  console.log(`変更後: ${TARGET_TITLE}`);
}
